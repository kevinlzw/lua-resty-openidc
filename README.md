...
-----END RSA PRIVATE KEY-----]],
             ...
             --   Signing algorithm for the client assertion JWT. Defaults to RS256 for private_key_jwt and HS256 for client_secret_jwt.
             --   When token_endpoint_auth_signing_alg_values_supported is advertised in discovery, the configured value must be supported by the OP.
             --client_jwt_assertion_alg = "PS256",
             --   Audience for the client assertion JWT. Defaults to the endpoint URL being called.
             --   Set this when the endpoint is reached through an internal URL but the OP expects its external token endpoint URL as audience.
